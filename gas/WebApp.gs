...
function deleteAssetRow_(ss, assetId) {
  deleteAssetDetails_(assetId);
  var found = findAssetInCategories_(ss, assetId);
  if (!found) return { error: "Asset not found" };
  found.sheet.deleteRow(found.rowIndex + 1);
  syncLocationAndPlantAssetSheets_(ss);
...
function doPost(e) {
  try {
    ...
    if (action === "delete_asset_redesigned") return json_(deleteRedesignedRow_("Assets", body.id, "Asset ID"));
