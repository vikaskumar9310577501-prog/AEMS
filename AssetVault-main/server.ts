...
import {
  ...
  getMaintenanceMachine,
  getMaintenanceMachineByAssetCode,
  upsertMaintenanceMachine,
  deleteMaintenanceMachine,
  listMaintenanceComplaints,
  getMaintenanceComplaint,
...
import { runMaintenanceCron } from "./server/maintenanceCron.js";
import {
  buildComplaintNotifyEmail,
  buildComplaintSolvedEmail,
  buildPreventiveSolvedEmail,
  buildTrendChangeEmail,
  pickMailIdentity,
  sendMaintenanceMail,
...
import { isDbMode, isSqlBackendUrl, isSqlMode, isSupabaseMode, SQL_BACKEND_URL } from "./server/sqlConfig.js";
...
function parseGasJsonResponse(text: string, label: string): Record<string, unknown> {
...
/** Call Apps Script /exec and parse JSON (follows Google's 302 result redirect). */
async function fetchAppsScriptJson(
  ...
  try {
    ...
    if (error instanceof Error && error.name === "AbortError") {
      throw new Error(`OTP request timed out after ${Math.round(timeoutMs / 1000)}s`);
    }
    throw error;
  } finally {
    clearTimeout(timer);
...
async function proxyToGas(payload: Record<string, unknown>, timeoutMs = 30000) {
  if (isDbMode() || isSqlBackendUrl(GAS_WEBAPP_URL)) {
    const { handleSqlAction } = await import("./server/sqlActions.js");
    return handleSqlAction(payload);
  }
  ...
  try {
    ...
    if (!hasSmtpConfigured()) {
      return res.status(503).json({
        error:
          "OTP mail uses nodemailer only. Set SMTP_EMAIL and SMTP_PASSWORD on the server (optional: SMTP_HOST, SMTP_PORT, OTP_FROM_EMAIL).",
      });
    }

    // Supabase / SQL: persist OTP + send via nodemailer (no Apps Script)
    if (isDbMode()) {
...
app.post("/api/upload", async (req, res) => {
  try {
    ...
    const url = useStoredUrl
      ? savedUrl
      : fileId
        ? drivePreviewUrl(fileId)
        : savedUrl;
    const viewUrl = useStoredUrl
      ? savedUrl || `/api/file/view?id=${encodeURIComponent(fileId)}`
      : fileId
...
app.get("/api/file/view", async (req, res) => {
  try {
    ...
    if (id.startsWith("local-")) {
      const { readLocalUpload } = await import("./server/sqlFiles.js");
      const local = await readLocalUpload(id);
...
async function fetchHeaders(): Promise<string[]> {
  ...
  try {
    let headers = parseHeaders(await gasGet(GAS_WEBAPP_URL));
    if (headers.length > 0) return headers;
    headers = parseHeaders(await gasGet(GAS_WEBAPP_URL, { action: "get_asset_headers" }));
    if (headers.length > 0) return headers;
  } catch (e) {
...
app.get("/api/data-sheets", async (req, res) => {
  try {
    ...
    const { buildDataSheets } = await import("./server/dataSheets.js");
    const sheets = await buildDataSheets();
...
app.get("/api/data-sheets.xlsx", async (req, res) => {
  try {
    ...
    const { buildDataSheets, sheetsToXlsxBuffer } = await import("./server/dataSheets.js");
    const sheets = await buildDataSheets();
...
function maintenancePlantName(plantCode: string) {
...
app.post("/api/maintenance/machines", async (req, res) => {
  try {
    ...
    const duplicate = existing.find(
      (m) =>
        m.machineType.toLowerCase() === machineType.toLowerCase() &&
        normalizeMachineNumber(m.machineNumber) === machineNumber &&
        String(m.plantCode || "").toLowerCase() === plantCode.toLowerCase()
    );
    if (duplicate) {
    ...
    const merged = isCustomTrend(trendMonths)
    ...
    const machine: MaintenanceMachine = {
      ...
      lastMaintenanceDate: String(body.lastMaintenanceDate || "").trim() || undefined,
      status: "Active",
...
app.put("/api/maintenance/machines/:id", async (req, res) => {
  try {
    ...
    const trendMonths =
      body.trendMonths !== undefined
        ? normalizeTrendMonths(body.trendMonths)
        : machineTrendMonths(current);
    const merged = isCustomTrend(trendMonths)
      ? mergeCustomPlan(
          nextMaintenanceDate,
          body.customPlanDates !== undefined ? body.customPlanDates : current.customPlanDates
        )
      : { nextMaintenanceDate, customPlanDates: [] as string[] };
    const updated: MaintenanceMachine = {
      ...
      responsibility:
        body.responsibility !== undefined
          ? String(body.responsibility || "").trim() || undefined
      ...
      lastMaintenanceDate:
        body.lastMaintenanceDate !== undefined
          ? String(body.lastMaintenanceDate || "").trim() || undefined
          : current.lastMaintenanceDate,
      status: (body.status as MaintenanceMachine["status"]) || current.status,
...
app.patch("/api/maintenance/machines/:id/trend", async (req, res) => {
  try {
    ...
    let mail: { ok: boolean; via?: string; error?: string; skipped?: boolean } = {
      ok: false,
      skipped: to.length === 0,
    };
    if (to.length) {
...
app.patch("/api/maintenance/machines/:id/details", async (req, res) => {
  try {
    ...
    if (body.equipmentName !== undefined) {
      patch.equipmentName = String(body.equipmentName || "").trim() || undefined;
    }
    if (body.department !== undefined) {
      patch.department = String(body.department || "").trim() || undefined;
    }
    if (body.responsibility !== undefined) {
      patch.responsibility = String(body.responsibility || "").trim() || undefined;
    }
...
app.post("/api/maintenance/complaints/public", async (req, res) => {
  try {
    ...
    const now = new Date().toISOString();
    const complaint: MaintenanceComplaint = {
    ...
    let mail: { ok: boolean; via?: string; error?: string } | undefined;
    if (to.length) {
    ...
    res.json({
      ...
      mailError: mail && !mail.ok ? mail.error : undefined,
    });
  } catch (error: any) {
...
app.get("/api/maintenance/dashboard", async (_req, res) => {
  try {
    ...
    const preventivePending = machines
      .map((m) => {
    ...
    const complaintRows = openComplaints.map((c) => {
      ...
      const pendingDays = Number.isNaN(reported.getTime())
        ? 0
        : Math.max(0, Math.floor((Date.now() - reported.getTime()) / (24 * 60 * 60 * 1000)));
      return {
        id: c.id,
...
function authorizeMaintenanceCron(req: { headers: Record<string, unknown>; query: Record<string, unknown> }): boolean {
...
app.get("/api/maintenance/cron", async (req, res) => {
  try {
    if (!authorizeMaintenanceCron(req as any)) {
...
async function startServer() {
  ...
  if (isSupabaseMode()) {
    const { initSupabase } = await import("./server/initSupabase.js");
    await initSupabase();
  } else if (isSqlMode()) {
    const { initSqlServer } = await import("./server/sqlPool.js");
    const { migrateLocalDataToSql } = await import("./server/sqlMigrate.js");
    await initSqlServer();
    await migrateLocalDataToSql();
  }
