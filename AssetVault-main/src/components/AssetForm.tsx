...
import {
  applyInHouseAssignmentPayload,
  type AssetAssignmentMode,
  resolveAssignmentMode,
} from "../lib/assetAssignment";
...
export default function AssetForm({ initialData, onSubmit, onCancel, loading, layout = "modal", prefillMainCategory, prefillAssetType, hideAssignee = false, allowedCategories: propAllowedCategories }: AssetFormProps) {
  ...
  useEffect(() => {
    ...
    setAssignmentMode(resolveAssignmentMode(initialData));
  ...
  const categoryPrefillAppliedRef = useRef(false);
  ...
  const [assignmentMode, setAssignmentMode] = useState<AssetAssignmentMode>(() =>
    resolveAssignmentMode(initialData)
  );
  ...
  const selectAssetType = (newType: AssetType, nextSubCategory = subCategoryForItAssetType(newType)) => {
    ...
    setFormData((prev) => {
      ...
      return {
        ...
        assetType: newType,
        subCategory: nextSubCategory,
        assetTypeId: typeDef?.id || (newType === "Desktop" ? "desktop" : newType === "Laptop" ? "laptop" : prev.assetTypeId),
        make,
        model,
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 3) {
      ...
      if (!hideAssignee && assignmentMode === 'in_house' && !formData.department?.trim()) {
        toast.error("Department is required for In House assets");
        return false;
      }

      if (hideAssignee || assignmentMode === 'in_house') {
        return true;
      }
  ...
  /** Apply dashboard category prefill once. Do not re-run when the user picks Desktop / IOD. */
  useEffect(() => {
    ...
    if (categoryPrefillAppliedRef.current) return;
    categoryPrefillAppliedRef.current = true;
    const firstSub = getDefaultSubCategory(prefillMainCategory);
    setFormData((prev) => {
      if (prev.mainCategory === prefillMainCategory && prev.subCategory === firstSub) return prev;
      return applyCategorySelection(prev, prefillMainCategory, firstSub, typeConfig);
    });
  }, [prefillMainCategory, prefillAssetType, allowedCategories, initialData?.id, typeConfig, getDefaultSubCategory]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const basePayload: AssetFormData = {
      ...merged,
      dynamicDetails: detailsForSave,
      assetTypeId: activeTypeDef?.id || formData.assetTypeId,
      status:
        !isSoftwareCategory && formData.maintenanceRequired === 'Yes'
          ? 'Under Maintenance'
        ...
        <div className="space-y-5 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          {(formData.mainCategory || "IT Assets") === "IT Assets" ? (
            <div className="space-y-4 pt-2">
              ...
              <>
              <div className="relative z-10 grid grid-cols-1 sm:grid-cols-3 gap-1 bg-slate-100 p-1 rounded-xl w-full border border-slate-200">
                {IT_PRIMARY_TYPES.map((type) => {
                  ...
                  const isSelected = isPeripheralGroup
                    ...
                    : formData.assetType === type && !isPeripheralGridOpen;
                  return (
                    <button
                      type="button"
                      key={type}
                      ...
                      className={cn(
                        "relative z-10 py-3 px-2 rounded-lg font-black text-xs uppercase tracking-widest transition-all cursor-pointer",
                        isSelected ? "bg-white text-blue-600 shadow-md ring-1 ring-slate-200" : "text-slate-400 hover:text-slate-600 hover:bg-white/70"
                      )}
                    >
                      {type}
                    </button>
