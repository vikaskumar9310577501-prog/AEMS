import { isDbMode, isSqlBackendUrl } from "./sqlConfig.js";
...
async function handleViaSql(payload: Record<string, unknown>): Promise<unknown> {
  const { handleSqlAction } = await import("./sqlActions.js");
  const parsed = await handleSqlAction(payload);
